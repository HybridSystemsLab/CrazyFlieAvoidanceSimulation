...
% r_omega = 25:27
% r_omeagadot = 28:30
function [t_out,j_out,x_a_out] = hybridVehicle(r, x_a, simTime, errorSkip)
    global HEQOptions QUAD_REFERENCE vehicleJump vehicleFlow xi_a omegaMapReset mapReset
    omegaMapReset = 1;
    mapReset = 1;
    xi_a = x_a';
    QUAD_REFERENCE = r;
    ...
    TSPAN = [0, simTime];
    JSPAN = [0, 50];
    [t_out,j_out,x_a_temp] = HyEQsolver(@f,@g,@C,@D,[x_a(end,:),0],TSPAN,JSPAN,1,HEQOptions);
    if(~exist('errorSkip','var'))
        errorSkip = 0; 
    end
    if(t_out(end) == 0 && errorSkip == 0)
        error("No flow in hybrid vehicle after %d jumps", j_out(end))
    end
    x_a_out = x_a_temp(:, 1:26);
end

function out = f(x)
    curtime = x(end);
    
    global InertialTensor Mass k k_omega k_z xi_a MaxMotorThrust d_xy
    
    omega_0 = omega_0Func(x);

    global mapReset
    persistent lastMap
    if isempty(lastMap) || mapReset == 1
        tempr = getReference(0);
        mapReset = 0;
        lastMap = [0, (tempr(1:6) - xi_a(1:6))', omega_0Func(xi_a)']';
    end
    if(x(end) > lastMap(1,end))
        lastVals = lastMap(:,end);
        lastt = lastVals(1);
        lastp_0 = lastVals(2:4);
        lastv_0 = lastVals(5:7);
        lastomega_0 = lastVals(8:10);
    elseif(size(lastMap(1,:)) < 2)
        tempr = getReference(0);
        lastt = 0;
        lastp_0 = tempr(1:3) - xi_a(1:3);
        lastv_0 = tempr(4:6) - xi_a(4:6);
        lastomega_0 = omega_0Func(xi_a);
    else
        lastVals = interp1(lastMap(1,:)', lastMap', x(end), 'previous')';
        if(lastVals(1) == x(end))
            lastVals = interp1(lastMap(1,:)', lastMap', x(end)-min(1e-9, 1e-9*x(end)), 'previous')';
        end
        lastt = lastVals(1);
        lastp_0 = lastVals(2:4);
        lastv_0 = lastVals(5:7);
    ...
    if(~ismember(lastMap(1,:),x(end)))
        lastMap(:,end+1) = [x(end), p_0', v_0', omega_0']';
    else
        [locMap, ~] = ismember(lastMap(1,:),x(end));
        lastMap(:,locMap);
        lastMap(:,locMap) = [x(end), p_0', v_0', omega_0']';
    end

    out = [pdot; vdot; Rdot; omegadot; zdot; 0; zeros(4,1); 1];
end
